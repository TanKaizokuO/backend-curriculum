 ...
 * opens one trace span with a child span for the database round trips that
 * matter. `observability.ts` holds the wiring; this file only calls it.
 *
 * Run it:
 *
 *     node src/migrate.ts
...
import { localLimiter, redisLimiter } from './rateLimit.ts';
...
// Postgres refused a new physical connection outright — not a checkout that
// timed out waiting, but the database itself out of room. Both mean the
// same thing to a caller: the connection pool is exhausted.
...
const pool = new Pool({ connectionString: config.databaseUrl, max: config.poolMaxSize });

/**
 * node-postgres's `Pool` has no built-in limit on how long a caller waits
 * for an available connection — only `connectionTimeoutMillis`, which
 * bounds opening a *new* physical connection, not queuing for one already
 * checked out. `psycopg_pool` ships a queue-wait timeout as `timeout=`;
 * node-postgres does not, so Lesson 12 builds one by hand: race every
 * query against a timer, and answer 503 if the timer wins. The query
 * already sent to Postgres keeps running either way — this stops the
 * caller from waiting for it, not the database from doing the work.
 */
export class PoolTimeoutError extends Error {}

function withAcquireTimeout<T>(operation: Promise<T>): Promise<T> {
  const { promise: timeout, reject } = Promise.withResolvers<never>();
  const timer = setTimeout(
    () => reject(new PoolTimeoutError('database connection pool exhausted')),
    config.poolAcquireTimeoutSeconds * 1000,
  );
...
pool.query = ((text: string, values?: unknown[]) =>
  withAcquireTimeout(originalQuery(text, values))) as typeof pool.query;
 ...
 *
 * The route template is not known until Express finishes matching, so it is
 * read from `req.route` inside the handler that runs after matching, via
 * `res.on('finish', ...)` — the same reason the Python middleware reads
 * `request.scope["route"]` only after `call_next`. Grouping `/bookmarks/17`
 * and `/bookmarks/42` under `/bookmarks/:id` keeps the metric's cardinality
 * bounded — one time series per route, not one per id ever requested.
 */
 ...
 *
 * Rate limited against brute force. `config.rateLimitBackend` picks where
 * the count lives. See `rateLimit.ts` and Lesson 12.
 */
app.post('/auth/login', route(async (req, res) => {
  ...
  const allowed =
    config.rateLimitBackend === 'local' ? localLimiter(clientIp) : await redisLimiter(redis, clientIp);
  if (!allowed) {
...
app.get('/bookmarks/search', route(async (req, res) => {
  ...
  const limit = Math.min(Number(req.query.limit ?? 10), 100);
  const skip = Number(req.query.skip ?? 0);
  const cacheKey = `search:${q}:${skip}:${limit}`;

  const cached = await redis.get(cacheKey);
  if (cached !== null) {
    res.json({ source: 'cache', results: JSON.parse(cached) });
    return;
  }

  const { rows } = await pool.query(SEARCH_SQL, [`${q}%`, limit, skip]);
  await redis.set(cacheKey, JSON.stringify(rows), { EX: SEARCH_CACHE_TTL_SECONDS });
...
app.post('/bookmarks', requireUser, route(async (req, res) => {
  ...
  const client = await withAcquireTimeout(pool.connect());
  try {
...
app.use((error: Error, _req: Request, res: Response, _next: NextFunction) => {
  const isPoolExhausted =
    error instanceof PoolTimeoutError || ('code' in error && error.code === TOO_MANY_CLIENTS);
  if (isPoolExhausted) {
    res.set('Retry-After', '1').status(503).json({ detail: 'database connection pool exhausted, try again shortly' });
    return;
  }
  console.error(error);
